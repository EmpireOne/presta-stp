...
class Ewayrapid extends PaymentModule
{
	...
	public function hookPayment($params) {
		...
		/*Loaded by Michael Z*/
		if ($params['cart']->id_customer == 0)
			return $this->display(__FILE__, 'ewayrapidstarter.tpl');
		/* Load objects */
...
//        ini_set("display_errors", 1);
        ...
        $Option1 = $result->Options->Option->Value;
        $id_cart = (int)(substr($Option1, 0, strpos($Option1, '_')));
        if (_PS_VERSION_ >= 1.5)
            Context::getContext()->cart = new Cart((int)$id_cart);
        $secure_cart = explode('_', $Option1);

        $this->validateOrder($cart->id, Configuration::get('PS_OS_PAYMENT'), $result->TotalAmount / 100, $this->displayName, $this->l('eWAY Transaction ID: ') . $result->TransactionID, array(), NULL, false, $secure_cart[2]);

        $confirmurl = __PS_BASE_URI__ . 'order-confirmation.php?id_cart='.intval($cart->id).'&id_module='.intval($this->id);
