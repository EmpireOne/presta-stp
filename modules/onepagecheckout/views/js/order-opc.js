...
function updateAddressSelection(idAddress_delivery, idAddress_invoice) {
    ...
    $.ajax({
        ...
        cache:false,
        dataType:"json",
        data:'ajax=true&method=updateAddressesSelected&id_address_delivery=' + idAddress_delivery + '&id_address_invoice=' + idAddress_invoice + '&token=' + static_token + _getAdditionalUrlParams(),
...
function updateCarrierSelectionAndGift() {
    ...
    $.ajax({
        ...
        cache:false,
        dataType:"json",
        data:'ajax=true&method=updateCarrierAndGetPayments&id_carrier=' + idCarrier + '&recyclable=' + recyclablePackage + '&gift=' + gift + '&gift_message=' + giftMessage + '&token=' + static_token + _getAdditionalUrlParams(),
        success:function (jsonData) {
            if (jsonData.hasError) {
                var errors = '';
                for (error in jsonData.errors)
...
function setPaymentModuleHandler() {
    ...
    var sel = $('form#paypalpro_form input#paypalpro_submit, form[name=cs_cardsave_direct_form] input#asubmit');
    ...
    $('.payment_module').not($('a#click_cs_cardsave_direct').parent()).unbind('click').click(_payment_module_handler);
...
function cartBlockCheckoutButtonHandler() {
    ...
    $.fn.scrollToElement('#opc_payment_methods', 1200);
