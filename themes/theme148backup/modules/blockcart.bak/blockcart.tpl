...
<div id="cart_block" class="block exclusive">
	...
	{/if}
		...
		{/if}
		<p id="cart-buttons">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process.php", true)}" class="button_mini" title="{l s='Cart' mod='blockcart'}">{l s='Cart' mod='blockcart'}<span>&nbsp;&gt;</span></a>{/if}
			<a href="{$link->getPageLink("$order_process.php", true)}{if $order_process == 'order'}?step=1{/if}" id="button_order_cart" class="{if $order_process == 'order-opc'}button{else}button_small{/if}" title="{l s='Check out' mod='blockcart'}">{l s='Check out' mod='blockcart'}<span>&nbsp;&gt;</span></a>
		</p>
	</div>
	</div>
</div>
