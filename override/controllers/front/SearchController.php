...
class SearchControllerCore extends FrontController
{
	...
	public function initContent()
	{
		...
		else if (($category_tag = urldecode(Tools::getValue('category'))) && !is_array($category_tag))
		{
			$nbProducts = (int)(Search::searchCategory($this->context->language->id, $category_tag, true));
			$this->pagination($nbProducts);
			$result = Search::searchCategory($this->context->language->id, $category_tag, false, $this->p, $this->n, $this->orderBy, $this->orderWay);
			Hook::exec('actionSearch', array('expr' => $category_tag, 'total' => count($result)));
			$this->context->smarty->assign(array(
				'category_search_tag' => $category_tag,
				'products' => $result, // DEPRECATED (since to 1.4), not use this: conflict with block_cart module
				'search_products' => $result,
				'nbProducts' => $nbProducts,
				'homeSize' => Image::getSize(ImageType::getFormatedName('home'))));
