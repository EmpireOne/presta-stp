...
class AdminAddressesControllerCore extends AdminController
{
	...
	public function __construct()
	{
		...
		foreach ($countries as $country)
			...
			//added 'other' and 'company_n' fields and concatenated phone numbers
		$this->fields_list = array(
			'id_address' => array('title' => $this->l('ID'), 'align' => 'center', 'width' => 25),
			'other' => array('title' => $this->l('Business Owner/Other')),
			...
			'phone_numbers' => array('title' => $this->l('Phone Number(s)')),
			'company_n' => array('title' => $this->l('Company')),
			'country' => array('title' => $this->l('Country'), 'width' => 100, 'type' => 'select', 'list' => $this->countries_array, 'filter_key' => 'cl!id_country'));

		parent::__construct();
		//I added the complex left join since the company duplicate field name in ps_customer and ps_address caused trouble on he one hand, and Prestashop doesn't play nice, and other field in the select query
		$this->_select = 'cl.`name` as country,`other`';
		$this->_join = '
			LEFT JOIN `'._DB_PREFIX_.'country_lang` cl ON (cl.`id_country` = a.`id_country` AND cl.`id_lang` = '.(int)$this->context->language->id.')
			LEFT JOIN `'._DB_PREFIX_.'customer` c ON a.id_customer = c.id_customer
			LEFT JOIN (SELECT `id_customer` as id_c,`company` as company_n from `'._DB_PREFIX_.'address`) t ON t.`id_c` = a.`id_customer`
			LEFT JOIN (SELECT `id_customer` as id_p, concat(`phone`,IF(`phone`=\'\',\'\',IF(`phone_mobile`=\'\',\'\',\',\')),`phone_mobile`) as phone_numbers from `'._DB_PREFIX_.'address`) p ON p.`id_p` = a.`id_customer`
		';
