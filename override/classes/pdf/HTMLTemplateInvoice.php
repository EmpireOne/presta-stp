...
class HTMLTemplateInvoiceCore extends HTMLTemplate
{
	...
	public function getContent()
	{
		...
		$shop_address_vertical = $this->getShopAddressVertical();
		
		$free_text_old_array = array("~line-break~","~bold-start~","~bold-end~");
		$free_text_new_array = array("<br />","<b>","</b>");
		$free_text_body = Configuration::get('PS_INVOICE_FREE_TEXT', (int)Context::getContext()->language->id, null, (int)$this->order->id_shop);
		$free_text_body = str_replace($free_text_old_array, $free_text_new_array, $free_text_body);
		$free_text_body_array = explode ("~create-second-column~", $free_text_body);
		...
		$this->smarty->assign(array(
			...
			'customer' => $customer,
			'shop_address_vertical' => $shop_address_vertical,
			'free_text_body_array' => $free_text_body_array
		));
	...
	protected function getShopAddressVertical()
	{
		$shop_address = '';
		if (Validate::isLoadedObject($this->shop))
		{
			...
			if (isset($shop_address_obj) && $shop_address_obj instanceof Address) {
				...
				$shop_address = substr(stristr($shop_address,"<br />"),6);
				$shop_address = str_replace(",<br />Australia","",$shop_address);
				return $shop_address;
			}
		}
