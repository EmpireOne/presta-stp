...
class FrontControllerCore extends Controller
{
	...
	*/
	...
	public function setMobileTemplate($template)
	{
		...
		//MZ customised a caching mechanism for the category tree
				// $categ_tree_dir = _PS_ROOT_DIR_.'/cache/sitemap_categ_tree';
		// $categ_tree_path = _PS_ROOT_DIR_.'/cache/sitemap_categ_tree/categ_tree.obj';
		// if(file_exists($categ_tree_path)) {
			// $categ_tree_unserialized = unserialize(file_get_contents($categ_tree_path));
			// $this->context->smarty->assign('categoriesTree', $categ_tree_unserialized);
		// } else {
			// if(!is_dir($categ_tree_dir)) {
				// mkdir($categ_tree_dir);
			// }
			// $categ_tree_unserialized = Category::getRootCategory()->recurseLiteCategTree(0);
			// file_put_contents($categ_tree_path,serialize($categ_tree_unserialized));
			// $this->context->smarty->assign('categoriesTree', $categ_tree_unserialized);
		// }
